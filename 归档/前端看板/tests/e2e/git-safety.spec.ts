...
test.describe("Git 五步确认", () => {
  test("未完成确认前不发送 Git 写请求，且不可越级", async ({ page }) => {
    ...
    await expect(page.getByText(/查看|第 1 步/)).toBeVisible();
  ...
  test("提交依次经过查看、填写、复核、确认、结果，并在确认后发送 confirmed", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /下一步|继续/ }).click();
    await expect(page.getByText(/复核|第 3 步/)).toBeVisible();
    await page.getByRole("button", { name: /下一步|继续/ }).click();
    await expect(page.getByText(/确认|第 4 步/)).toBeVisible();
  ...
  test("冲突状态会明确阻止危险 Git 操作并显示处理提示", async ({ page }) => {
    ...
    await page.goto("/");
    await expect(page.getByText(/冲突|必须人工处理/)).toBeVisible();
