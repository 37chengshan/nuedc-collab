...
import { generateTaskId, isEventId, isIdeaId, isIssueId, isTaskId, nowIso } from '../src/ids.js';
import { computeRevision, stableStringify } from '../src/json.js';
import { resolveRepoPath } from '../src/paths.js';
import { validateLocalSettings, validateTask, systemCanvasSchema } from '../src/schemas.js';
import Ajv from 'ajv';
  ...
  it('校验任务与本机设置', () => {
    ...
    expect(
      validateLocalSettings({
        ...
        autoFetchIntervalSeconds: 60,
        motionLevel: 'system',
        confirmGitWrites: false as true,
      }),
    ).toBe(false);
  });
});
