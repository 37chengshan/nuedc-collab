...
截至 2026-07-31，C 题 UWB 必须以以下实测结论为准，不得沿用旧计划中的理想化结论：

- `apps/uwb-recorder/data/captures/` 的运行模型使用 68 个最终测点：旧数据 18 点、新结构化数据 50 点；基站间距 250 mm，当前完整地址为 `0100` 和 `0200`，按低 4 位 `keyId`、独立 UART 链路和 120 ms 时间窗同步。
- 独立指标仍使用未包含两个最终 1.5 m 补测点的 66 点模型：距离最大误差约 0.20 m、P95 约 0.099 m。两个 1.5 m 点在独立泛化时误差约 0.50 m/0.405 m，纳入 68 点运行模型后只证明已采位置可拟合，不能冒充独立验证。
- OLED、UWB Lab 和数字钥匙工作台统一每 500 ms 更新。两基站距离增加稳定器：小变化按 25% 跟随，大于 0.18 m 的突变连续 3 次一致才接受；仍需实机动态进出和长期静止验收。
- 1 m 区域当前拟合良好；2 m 区域部分 ±20°数据与 1.5 m 补测点读数重叠，留一评估最大误差约 0.27 m、P95 约 0.24 m，必须继续补采边界和重叠区。
- 两基站角度独立评估最大误差约 45.3°、P95 约 25.3°，结论为“不满足比赛精度”。角度可以持续显示，不可信时保持上一值，但两基站角度不得参与开锁；可信角度转入第三基站非共线布局。
- 固件定位接口支持 2～4 基站和三/四基站异常降级，但当前目标板只配置了两路 UWB UART。第三、第四基站必须各使用独立 UART 和实测天线中心坐标。
- L1 屏幕、L2 双路监看、L3 拨码身份、L4 完整门锁四层 HEX 已构建通过；尚未完成目标板烧录和实机连续运行，不得写成“上板通过”。
...
5. `T-20260731-2MBD`：补采 1 m/2 m 边界与 1.5/2 m 重叠区。
6. `T-20260731-A3VW`：增加第三基站并完成可信角度标定。
