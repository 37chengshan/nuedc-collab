...
def normalize_usb_number(value: object) -> str:
    """Normalize ioreg decimal/hex USB numeric properties to four hex digits."""
...
def detect_linux() -> list[Probe]:
    ...
    for name in os.listdir(root):
        ...
        values: dict[str, str] = {}
        for field in ("product", "manufacturer", "serial", "idVendor", "idProduct"):
        ...
        probes.append(
            Probe(
                ...
                usb_id=usb_id,
                serial_number=values.get("serial", ""),
                serial_ports=[],
                confidence=confidence,
                recommended_backend=backend,
                recommended_config=config,
...
def darwin_ioreg_devices() -> list[dict[str, str]]:
    """Read USB objects from ioreg without requiring System Information access."""
    output = run_command(["ioreg", "-p", "IOUSB", "-l", "-w", "0"])
    # ioreg prefixes nested devices with tree characters ("|     +-o"),
    # so matching only whitespace would miss XDS110 behind a USB hub.
    blocks = re.split(r"(?m)(?=^[^\n]*\+-o )", output)
    devices: list[dict[str, str]] = []
    for block in blocks:
        if "idVendor" not in block or "idProduct" not in block:
            continue
        ...
        devices.append(
            {
                "product": property_value("USB Product Name") or property_value("Product Name"),
                "manufacturer": property_value("USB Vendor Name") or property_value("Manufacturer"),
                "serial": property_value("USB Serial Number") or property_value("Serial Number"),
                "idVendor": property_value("idVendor"),
                "idProduct": property_value("idProduct"),
            }
        )
    return devices


def darwin_serial_ports() -> list[str]:
    ports = set(glob("/dev/cu.*")) | set(glob("/dev/tty.*"))
    return sorted(path for path in ports if path.rsplit("/", 1)[-1] not in {"cu.Bluetooth-Incoming-Port", "tty.Bluetooth-Incoming-Port"})


def detect_darwin() -> list[Probe]:
    serial_ports = darwin_serial_ports()
    probes: list[Probe] = []
    for device in darwin_ioreg_devices():
        vendor = normalize_usb_number(device.get("idVendor"))
        product = normalize_usb_number(device.get("idProduct"))
        usb_id = f"{vendor}:{product}" if vendor and product else ""
        texts = [value for value in (device.get("product", ""), device.get("manufacturer", "")) if value]
        ...
        probes.append(
            Probe(
                kind=kind,
                display_name=mapped_name or device.get("product", "") or kind,
