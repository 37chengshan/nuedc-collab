...
static bool test_welcome_blinks_and_short_beeps_once(void)
{
    LockOutputBehavior behavior;
    LockOutputSnapshot logical;
    LockPhysicalOutputs physical;
    ...
    physical = lock_output_behavior_update(&behavior, &logical, 179U);
    TEST_ASSERT(physical.buzzer_on);
    physical = lock_output_behavior_update(&behavior, &logical, 180U);
    TEST_ASSERT(!physical.buzzer_on);

    physical = lock_output_behavior_update(&behavior, &logical, 250U);
    TEST_ASSERT(!physical.green_on);
    ...
    memset(&logical, 0, sizeof(logical));
    (void)lock_output_behavior_update(&behavior, &logical, 600U);
    logical.welcome_output = true;
    physical = lock_output_behavior_update(&behavior, &logical, 700U);
    TEST_ASSERT(physical.buzzer_on);
    return true;
}
...
int main(void)
{
    ...
    if (!test_welcome_blinks_and_short_beeps_once()) {
        failures++;
    }
