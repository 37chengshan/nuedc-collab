...
该分配按实际地猛星排针图选择：`PB8/PB9` 和 `PA12/PA13/PA17` 均已引出，
不再使用板上未引出的 `PB10/PB11/PB14`。屏幕供电和背光按模块标称电压
连接，并与 MCU 共地。
...
| `1:xxx 2:xxx` | UART1/UART3 最近收到的两路原始距离，单位 cm；`---` 未收到，`000` 收到但模块报 0 cm |
...
原始通道值会保留 1.5 秒，便于观察低速输出；零距离只用于诊断显示，不参与
定位、区域判断或开锁。
