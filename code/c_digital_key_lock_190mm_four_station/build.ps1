param(
    [ValidateSet("All", "App", "ScreenDemo", "DipTest", "Monitor", "Identity", "Full")]
    [string]$Variant = "App",
...
$OutputName = switch ($Variant) {
    "ScreenDemo" { "c_digital_key_lock_190mm_l1_screen" }
    "DipTest" { "c_digital_key_lock_190mm_dip_test" }
    "Monitor" { "c_digital_key_lock_190mm_l2_monitor" }
    "Identity" { "c_digital_key_lock_190mm_l3_identity" }
    "Full" { "c_digital_key_lock_190mm_four_uwb_full" }
    default { "c_digital_key_lock_190mm_four_uwb" }
}
...
$AppSources = @(
    ...
    "uwb_four_station_estimator.c",
    "four_station_model_data.c"
)
...
$Sources = $SharedDisplaySources + $(
    if ($Variant -eq "ScreenDemo") {
        $DemoSources
    } elseif ($Variant -eq "DipTest") {
        $DipTestSources
    } else {
        $AppSources
    }
