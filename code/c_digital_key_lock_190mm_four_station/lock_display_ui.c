...
static void draw_static_frame(LockDisplayUi *ui)
{
    ...
    st7735s_draw_text(display, 22, 4, "DIGITAL KEY", UI_COLOR_WHITE,
                      UI_COLOR_PANEL, true);
    st7735s_draw_text(display, 104, 4, "D6", UI_COLOR_CYAN,
                      UI_COLOR_PANEL, true);
    st7735s_draw_line(display, 0, 14, 127, 14, UI_COLOR_CYAN);

    draw_label(display, 19, "DIP SET");
                             ...
                             const LockDisplayModel *model)
{
    static const int16_t arc_points[][2] = {
        {36, 91}, {40, 83}, {48, 77}, {56, 74}, {64, 73},
        {72, 74}, {80, 77}, {88, 83}, {92, 91},
    };
    St7735s *display = ui->display;
    char angle[LOCK_DISPLAY_ANGLE_TEXT_CAPACITY];
    char channels_ab[LOCK_DISPLAY_CHANNEL_TEXT_CAPACITY];
    char channels_cd[LOCK_DISPLAY_CHANNEL_TEXT_CAPACITY];
    uint16_t color =
    ...
    lock_display_format_channels(model, channels_ab);
    lock_display_format_channel_pair(model, 2U, channels_cd);
    st7735s_draw_text(display, 7, 53, channels_ab, channel_color,
                      UI_COLOR_PANEL, true);
    st7735s_draw_text(display, 7, 63, channels_cd, UI_COLOR_MUTED,
                      UI_COLOR_PANEL, true);
    for (index = 1U;
    ...
    if (model->position.angle_valid || model->position.angle_held) {
        ...
        end_x = (int16_t)(64.0f + sinf(radians) * 18.0f);
        end_y = (int16_t)(91.0f - cosf(radians) * 18.0f);
        st7735s_draw_line(display, 64, 91, end_x, end_y, color);
        st7735s_fill_rect(display, 62, 89, 5, 5, color);
    } else {
        st7735s_draw_line(display, 58, 89, 70, 89, UI_COLOR_YELLOW);
    }

    lock_display_format_angle(model, angle);
    st7735s_draw_text(display, centered_x(angle, 1U), 99, angle, color,
                      UI_COLOR_PANEL, true);
}
                            ...
                            const LockDisplayModel *model)
{
    ...
    uint16_t background;
    uint16_t foreground = UI_COLOR_WHITE;

    if (model->monitor_only) {
        background = UI_COLOR_BLUE;
    } else {
        switch (model->state) {
        case LOCK_STATE_UNLOCKED:
            background = UI_COLOR_GREEN;
            foreground = UI_COLOR_BLACK;
            break;
        case LOCK_STATE_WELCOME:
            background = UI_COLOR_YELLOW;
            foreground = UI_COLOR_BLACK;
            break;
        case LOCK_STATE_DENIED:
        case LOCK_STATE_CALIBRATION_ERROR:
            background = UI_COLOR_RED;
            break;
