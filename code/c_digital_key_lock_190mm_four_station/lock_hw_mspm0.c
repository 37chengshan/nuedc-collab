...
#define HARDWARE_UWB_CHANNEL_COUNT 4U
...
{
    /*
     * Do not gate a receive by one exact IIDX value. UART0 and UART1 use
     * the same RX FIFO configuration; draining a non-empty FIFO is the
     * authoritative indication that a byte has arrived.
     */
...
static void poll_uart_channel(uint8_t channel)
{
    /*
     * UART interrupt delivery is the normal path. This short FIFO poll is a
     * bounded fallback for either channel. Logical B is intentionally mapped
     * to the physically observed UART3 line; UART0 is retained as logical D.
     * Disable only the matching NVIC line while this code becomes the
     * producer of that ring buffer.
     */
    if (channel == 0U) {
        NVIC_DisableIRQ(UWB_CH1_INST_INT_IRQN);
        capture_uart(0U, UWB_CH1_INST);
        NVIC_EnableIRQ(UWB_CH1_INST_INT_IRQN);
    } else if (channel == 1U) {
        NVIC_DisableIRQ(UWB_CH4_INST_INT_IRQN);
        capture_uart(1U, UWB_CH4_INST);
        NVIC_EnableIRQ(UWB_CH4_INST_INT_IRQN);
    } else if (channel == 2U) {
        NVIC_DisableIRQ(UWB_CH3_INST_INT_IRQN);
        capture_uart(2U, UWB_CH3_INST);
        NVIC_EnableIRQ(UWB_CH3_INST_INT_IRQN);
    } else if (channel == 3U) {
        NVIC_DisableIRQ(UWB_CH2_INST_INT_IRQN);
        capture_uart(3U, UWB_CH2_INST);
...
void UWB_CH4_INST_IRQHandler(void)
{
    capture_uart(1U, UWB_CH4_INST);
}
...
void lock_hw_init(void)
{
    ...
    DL_GPIO_setPins(LOCK_OUTPUTS_PORT, LOCK_OUTPUTS_BUZZER_PIN);
    DL_GPIO_clearPins(LOCK_RELAY_PORT, LOCK_RELAY_DRIVE_PIN);
    ...
    NVIC_ClearPendingIRQ(UWB_CH4_INST_INT_IRQN);
    NVIC_EnableIRQ(UWB_CH1_INST_INT_IRQN);
    NVIC_EnableIRQ(UWB_CH2_INST_INT_IRQN);
    NVIC_EnableIRQ(UWB_CH3_INST_INT_IRQN);
    NVIC_EnableIRQ(UWB_CH4_INST_INT_IRQN);
...
void lock_hw_apply_outputs(const LockOutputSnapshot *outputs)
{
    ...
    if (outputs != NULL) {
        ...
        write_output_pin(
            LOCK_OUTPUTS_WELCOME_LED_PIN, outputs->yellow_led);
        if (physical.buzzer_on) {
            DL_GPIO_clearPins(
                LOCK_OUTPUTS_PORT, LOCK_OUTPUTS_BUZZER_PIN);
        } else {
            DL_GPIO_setPins(
                LOCK_OUTPUTS_PORT, LOCK_OUTPUTS_BUZZER_PIN);
        }
        if (physical.lock_on) {
            DL_GPIO_setPins(LOCK_RELAY_PORT, LOCK_RELAY_DRIVE_PIN);
        } else {
            DL_GPIO_clearPins(LOCK_RELAY_PORT, LOCK_RELAY_DRIVE_PIN);
        }
    }
