...
static bool test_fsm_authorization_and_dropout(void)
{
    ...
    /*
     * The fixed design-key ID is 0001.  Any other DIP setting must remain
     * denied even when all four distances indicate the unlock zone.
     */
    lock_fsm_init(&state_machine);
    {
        LockPositionSolution fixed_design_key =
            position(1U, true, 800.0f, 0.0f,
                     LOCK_LOCALIZATION_FOUR_ANCHOR, 4U);
        output = lock_fsm_update(&state_machine, &fixed_design_key, 2U,
                                 &config, 600U);
        TEST_ASSERT(!output.authorized);
        TEST_ASSERT(output.state == LOCK_STATE_DENIED);
        TEST_ASSERT(output.red_led);
