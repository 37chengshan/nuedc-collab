...
| 类型 | 目录与命名 | 谁能改主记录 | 其他成员怎么参与 |
| --- | --- | --- | --- |
| 任务 | `比赛管理/任务/T-日期-四位码.json` | 当前 `owner`；未认领时由创建者/队长明确认领 | 新建事件记录测试、评论、交接 |
| 问题 | `比赛管理/问题/I-日期-四位码.json` | 当前 `owner` | 追加事件、提供复现证据 |
| 想法 | `比赛管理/想法/A-日期-四位码.json` | 未认领想法可由作者维护；已认领后由 `owner` | 追加决策/评论事件 |
| 事件 | `比赛管理/事件/E-日期-时间-四位码.json` | 创建者只写自己的新事件 | 不回写父记录的事件数组 |
| 成员 | `比赛管理/成员/<GitHub用户名>.json` | 对应成员或队长按流程维护 | 不替别人伪造身份 |
...
Agent CLI 从 `.本机配置/settings.json` 读取本机 `githubUsername`，请求不应手工伪造 `actor`。用户名未登记为 active，或与本机设置不一致时，停止写入并先修正成员登记。任何人不能替 AI 或命令行跳过 owner 规则，也不能让自动化静默确认 Git 写操作。
